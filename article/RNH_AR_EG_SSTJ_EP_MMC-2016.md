...
can be extended to take into account the contribution of free water diffusion by representing
...
used to initialize the non-linear solver (see below).

**Non-Linear Least Square Solution (NLS)**. As suggested by Hoy and colleagues
[@Hoy2014-lk,], the initial guess for the non-linear solver 
was set to the values estimated from the WLS approach. To improve the speed of
...
converted to $f_t = \arcsin (2f-1) + \pi / 2$. 
...
constraints similarly to the original article, however
...
is not used by default and is not compatible with the analytical jacobian
derivation.
...
the water volume fraction close to zero. To remove these problematic cases, for all voxels with
standard DTI mean diffusivity values larger than $2.7 \times 10^{-3} mm^{2}.s^{-1}$, 

stuff here

the free
water volume fraction is set to one while all other diffusion tensor
parameters are set to zero. This mean diffusivity threshold was arbitrarily adjusted to 90%
...
implemented procedures also requires the python package NumPy (http://www.numpy.org/),
...
directions with diffusion weighting b-values of 500 and 1500 $s.mm^{-2}$ and with six $b=0 s.mm^{-2}$ images.
...
Table: Eigenvalues used for the simulations {#tbl:table}
...
For each FA value, 11 degrees of free water contamination were
evaluated (f values were linearly spaced from 0 to 1). To assess the robustness of the
procedure, Rician noise with signal-to-noise ratio (SNR) of 40 relative to the b-value = 0 images was
used. 

stuff here

For each FA and f-value pair, simulations were performed for 120
different diffusion tensor orientations. Simulations for each diffusion tensor
orientation were repeated 100 times for a total of 12 000 simulations
iterations for each FA and f-value pair.

## In vivo data

Similarly to the original article, the procedures are also tested using in vivo human 
brain data [@valabregue2015], that can be automatically
downloaded by Dipy's functions.

stuff here

The original dataset consisted of 74 volumes of images acquired for a
b-value of $0 s.mm^{-2}$ and 578 diffusion weighted images acquired along 16 diffusion gradient directions
...
The results from the Monte Carlo simulations are shown in Figure @fig:simulations. AS reported similarly
in the original article, FA values estimated using the free water elimination model match the tissue's ground truth
values for free water volume fractions $f$ ranging from 0 to 0.7 (top panel of
Figure @fig:simulations). However, FA values seem to be overestimated for higher volume fractions. This bias is more
prominent for lower FA values, in which overestimation is visible from lower free water volume
...
for the five different FA ground truth levels plotted as function of the ground truth water volume fraction.
The bottom panels show the estimated volume fraction $f$ median and interquartile range as function of its ground truth values
...
in an average desktop and laptop PC (~2GHz processor speed (how many cores, parallel or not?)), 
hile the reported processing time by Hoy et al. was around 20 hours. 

sentence to say this is a ballpark estimate, since they used a different numerical (or not) fitting, different platform, code, dataset, etc.

The free water elimination model seems to produce higher values
...
and might be removed by excluding voxels with measured water volume fractions above a reasonable
